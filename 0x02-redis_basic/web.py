...
def data_cacher(method: Callable) -> Callable:
    '''Decorator to cache the output of a method that fetches data from a
    URL.

    Args:
        method (Callable): The function to be decorated, which fetches data
        from a URL.

    Returns:
        Callable: A wrapped function that caches the result of the original
        function.
    '''
    @wraps(method)
...
def get_page(url: str) -> str:
    ...
    Uses the `data_cacher` decorator to cache the response and track
    requests.
    Args:
